...
\end{aligned}
\]
which extends to a homomoprhism $\overline\psi:S[x]\rightarrow R[x]$, under which $\overline f=\prod_{i=1}^d(x-y_i)$ is mapped to $f$ and $\overline g=\prod_{j=1}^e(x-z_j)$ is mapped to $g$. It's then clear that $\psi\left(\re_{\overline f,\overline g}\right)=\re_{f,g}$.
...
\]
for some contant $c$. Substituting $y_i=1,z_j=0$ for all $i,j$, one has $\overline f=(x-1)^d$ and $\overline g=x^e$, so
\[
...
\begin{prop}
If $g\in R[x]$ has $\deg g\geq 1$, then $R_{x-c,g(x)}=g(c)$.
\end{prop}
\begin{proof}
...
\]
and clearly the root $[]cd-af,ae-bd]$ has multiplcity 1. 
...
\begin{prop}
$C,D$ are transverse at $p\iff T_pC$ and $T_pD$ are different.
\end{prop}
\begin{proof}
